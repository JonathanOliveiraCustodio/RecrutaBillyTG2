...
<body>
	<script
		src="${pageContext.request.contextPath}/resources/js/categoriaProduto.js"></script>
	<script
	...
	<div class="container py-4 text-center d-flex justify-content-center"
		align="center">
		<c:if test="${nivelAcesso == 'admin' && not empty categoriaProdutos}">
			<div class="table-responsive w-100">
				<table class="table table-striped">
					...
					<tbody class="table-group-divider">
						<c:forEach var="cp" items="${categoriaProdutos }">
							<tr>
								<td style="text-align: center;">
									<button class="btn btn-outline-dark" name="opcao"
										value="${cp.codigo}" onclick="editarCategoriaProduto(this.value)"
										${cp.codigo eq codigoEdicao ? 'checked' : ''}>
						...
						<path fill-rule="evenodd"
								...
								<td><c:out value="${cp.codigo }" /></td>
								<td><c:out value="${cp.nome }" /></td>
								<td style="text-align: center;">
									<button class="btn btn-danger"
										onclick="excluirCategoriaProduto('${cp.codigo}')">Excluir</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
